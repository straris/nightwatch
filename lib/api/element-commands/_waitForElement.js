...
class WaitForElement extends EventEmitter {
  ...
  command(selector, milliseconds, callbackOrAbort) {
    ...
    if (selector.includes('//')) {
      this.client.locateStrategy = 'xpath';
      this.locateStrategy = 'xpath';
    } else {
      this.client.locateStrategy = 'css selector';
    ...
    if (typeof arguments[1] === 'function') {
      ...
      this.cb =
        (typeof callbackOrAbort === 'function' && callbackOrAbort) ||
        function() {};
    }
  ...
  createElement(selector) {
    this.element = Element.createFromSelector(
      selector,
   ...
   */
  locateElement() {
    return this.getProtocolCommand()
      .then(result => {
        ...
        let elementResult = this.transport.resolveElement(
          result,
          this.element,
          true
        );
  ...
  checkElementVisible() {
    this.transport
      .executeProtocolAction('isElementDisplayed', [this.elementId])
      .then(result => {
        let now = new Date().getTime();
        if (
          this.transport.isResultSuccess(result) &&
          this.isElementVisible(result)
        ) {
          // element was visible
  ...
  setMilliseconds(timeoutMs) {
    ...
    if (typeof globalTimeout !== 'number') {
      throw new Error(
        'waitForElement expects second parameter to have a global default ' +
